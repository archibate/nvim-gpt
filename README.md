...
- If you want to edit multi-line questions, simply press `<CR>` again (i.e. empty arguments for `:GPT`) and you will be directed into a new buffer for editing questions, which is in normal mode by default. After finished editing the question, press `<CR>` in normal mode to submit. The question will be remained when the next time you enter the multi-line editor for convenience of re-editing and re-asking GPT.
...
- If you dislike the current answer, you may press 'r' to ask GPT to regenerate a different solution.
